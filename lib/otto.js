            ...
            this.applyFilter = function() {
                ...
                for (var i = 0; i < this.allChoices.length; i++) {
                    ...
                    if (choice === Object(choice)) {
                        if (choice.label) matchOn = label = choice.label;
                        if (choice.matchOn) matchOn = choice.matchOn;
                        if (!choice.label && !choice.matchOn) throw 'Otto Error: Invalid choice object!';
                    } else {
                ...
                for (var i = 0; i < Math.min(maxResults, tempChoices.length); i++) {
                    ...
                    if (tempChoices[i] === Object(tempChoices[i])) {
                        ...
                        if (emText[i].index < 0) {
                            // No Emphasis Text, just append string without bolding
                            this.filteredChoices.push(tempChoices[i].label);
                            continue;
                        } else {
                            choice = tempChoices[i].label;
                        }
                    } else {
                        choice = tempChoices[i];
                    }

                    this.filteredChoices.push(
                        choice.slice(0, emText[i].index)
                            + '<b>'
                            + choice.slice(emText[i].index, emText[i].index + emText[i].len)
                            + '</b>'
                            + choice.slice(emText[i].index + emText[i].len)
                    );
                }
            };
